...
다음 목표 - 게시판 정보를 db에서 처리하기 offset과 rimit를 활용. 사이드메뉴 항목추가?. 주문조회페이지.

2021.12.17 - 로고상표를 Ferry로 변경 이미지는 만들지않고 텍스트로 처리. 비로그인 중 장바구니 클릭 시 접근하지 못하도록했음. 쿠키에 담겨진 후 로그인 후에 추가하는 방안은 미룰 것.
